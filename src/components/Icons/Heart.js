...
const Icon = ({ size=22, filled=false }) => {
  ...
  return (
    <svg id="svg2" width={`${size}px`} height={`${size * 0.9}px`} viewBox={`-1 0 ${size} ${size * 0.9 - 0.8}`} xmlns="http://www.w3.org/2000/svg" version="1.0">
      <g id="layer1">
        <path id="path2417" fill={fillColor} stroke="#000000" strokeWidth="37.6152" transform={`scale(${scaleRatio})`}
